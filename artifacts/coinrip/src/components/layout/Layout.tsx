...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  // Auto-open username setup for new users
  useEffect(() => {
    if (needsDisplayName) {
      // Small delay so the login modal can close first
      const t = setTimeout(() => setIsUsernameOpen(true), 600);
  ...
  return (
    <div className="min-h-[100dvh] bg-background text-foreground pb-20 flex flex-col max-w-md mx-auto relative overflow-hidden border-x border-border shadow-2xl">
      ...
      <UsernameSetupModal open={isUsernameOpen} onOpenChange={setIsUsernameOpen} />
    </div>
  );
}
